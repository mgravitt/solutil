...
pub fn print_sol_transfer_history(solana_rpc_url: &str, solana_address: &str) -> Result<(), Box<dyn std::error::Error>> {
    
    println!("Print sol history for {} via {}", solana_address, solana_rpc_url);
    ...
    for signature in signatures_rpc_response.result {
        ...
        if is_sol_transfer(&json) {
            if let Some(sol_transfer) = SolTransfer::from_json(&json) {
                table.add_row(row!(
                    sol_transfer.transaction_id, 
                    sol_transfer.sender, 
                    sol_transfer.receiver, 
                    sol_transfer.amount, 
                    sol_transfer.timestamp
                ));
            }
        }
    }
